import b5 from 'bigfivequestionscustom-2020'
...
export const formatId = id => /^((http|https):\/\/)/.test(id) ? id.replace((process.env.NODE_ENV === 'development' ? 'http://localhost:8080' : 'https://test-mypersonality.com') + '/result/', '').replace(' ', '').toLowerCase() : id ? id.replace(' ', '') : id
